...
%
%  If the stim's displaystruct has a userfield entry that specifies 'pauseRefresh',
%  then those values are used for pauseRefresh.
%  
%    
...
for i=1:numStims(stimScript),
	...
	if NS_PTBv>=3,
		thedss{i}.clut_bg = mergeluts(currLut,thedss{i}.clut_bg/255);
		if iscell(thedss{i}.clut),
        ...
        moviep{i} = MovieParams2MTI(thedss{i},thedfs{i});
        moviec{i} = ClipRgnParams2MTI(thedss{i},thedfs{i});
	end;
end;
...
for i=1:length(dispOrder)
	...
	if (strcmp(ds.displayType,'CLUTanim'))|(strcmp(ds.displayType,'Movie'))|(strcmp(ds.displayType,'MultiMovie')),
		...
		if isfield(ds,'userfield'),
			if isfield(ds.userfield,'pauseRefresh'),
				pauseRefresh = ds.userfield.pauseRefresh;
			end;
		end;
	...
	MTI{i}.MovieParams = moviep{dispOrder(i)};
	MTI{i}.ClipRgnParams = moviec{dispOrder(i)};
end;

...
han.cyclespersec=uicontrol(edit,'position',[90   380+sh   70    22],'string','4');
...
str=[str sprintf('%s\n','w             -   Write mask tiff image to Desktop')];
...
%                 sca; keyboard;
		    ...
		    case 'w',
			[xxxx,yyyy] = meshgrid(1:StimWindowRect(3),1:StimWindowRect(4));
			ctr = [mean(myrect([1 3])) mean(myrect([2 4]))];
			x_axis = rad; %diff(myrect([1 3]))/2;
			y_axis = rad; %diff(myrect([2 4]))/2;
			img = (((xxxx-ctr(1))/x_axis).^2 + ((yyyy-ctr(2))/y_axis).^2) < 1;
			img = uint8(255*img);
			if isunix,
				fname = ['~' filesep 'Desktop' filesep 'quickRFshot ' datestr(now) '.tiff'];
				disp(['going to write file ' fname '.']);
				imwrite(img,fname,'tiff');
			else,
				disp(['Writing not supported on this platform.']);
			end;

function initstims_alt(varargin)
% INITSTIMS_ALT - Put a stimulus computer in a remote mode to receive commands from a server
%
%   INITSTIMS_ALT
%
%   Puts a stimulus computer into remote mode to receive commands from a server.
%
%   One can also call INITSTIMS_ALT(N) if the stimulus computer is not the only 
%   stimulus computer listening. If each stimulus computer listens with a unique
%   number N, the stimulus computers will not interfere with one another.
%
...
cd(theDir);
if exist(['runit' nstr '.m'])==2, delete(['runit' nstr '.m']), end;
...
case 'filesystem',
	while 1,  % needs control-C to exit
		...
		if ~isempty(txt),
			...
			delete(['runit' nstr '.m']);
			if exist(['toremote' nstr])==2, delete(['toremote' nstr]); end;
		end;
	end;
case 'sockets',
	pnet('closeall');

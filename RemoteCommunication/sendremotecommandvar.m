function [b,vars] = sendremotecommandvar(strs,invarnames,invars,number)
...
%
%  [B,VARS] = SENDREMOTECOMMANDVAR(STRS,INVARNAMES,INVARS,NUMBER) 
%    will write the file to the 'toremote_NUMBER' and use 
%    'runit_NUMBER.txt' and 'gotit_NUMBER' to exchange information.
%    In this way, one can communicate with more than one server at once.
%
...
strs = cat(ind,{['load(''toremote' nstr ''',''-mat'');']},strs);
...
if checkremotedir(pathstring), % directory exists
	...
	fout  = [pathn 'fromremote' nstr];
	fin  =  [pathn 'toremote' nstr];

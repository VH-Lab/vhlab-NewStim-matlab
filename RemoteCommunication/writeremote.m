function b = writeremote(strs, number)
...
%
%  If NUMBER is present, then the file is written to runit_NUMBER.m,
%  so that multiple computers can receive instructions.
%
...
elseif exist(fname), % check to see that any previous scripts are finished
	...
	if b==0,
        ...
        delete(fname);  % runit.m is simply not cleaned up so clean it
	end;
end;

function F = findtransitions(BL, colors_from, colors_to, tolerance)
% FINDTRANSITIONS - Find transitions between specific colors for BLINKINGSTIM
%
%  F = FINDTRANSITIONS(BL, COLORS_FROM, COLOR_TO)  or
%  F = FINDTRANSITIONS(BL, COLORS_FROM, COLOR_TO, TOLERANCE)
%
%  For a BLINKINGSTIM BL, finds frame numbers that correspond to a
...
%	bl = blinkingstim('default');
%       colors_from = [ 0 0 0 ]; % black
%       colors_to   = [ 255 255 255 ]; %white
%       F = findtransitions(bl,colors_from,colors_to);
%
...
p = getparameters(BL);
colors = [p.BG ; p.value];
V = [ones(X*Y,1) getgridvalues(BL)]; % include background transition first

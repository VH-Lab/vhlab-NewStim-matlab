function [done,stamp,infoO] = customdraw(ggs, info, MTI)
% CUSTOMDRAW - Custom draw routine for glidergridstim
%
%  [DONE,STAMP,INFO] = CUSTOMDRAW(GGS, INFO, STIMWINDOW,...
%                    MTI)
%
%  This custom draw routine is used for display GLIDERGRIDSTIM stimuli.
%
...
if isempty(info),
	...
	for i=1:X, % could do this without for loops, save for later
		for j=1:Y,
			gridrects(:,end+1) = ...
				[0*1+(i-1)*GGSparams.pixSize(1) 0*1+(j-1)*GGSparams.pixSize(2) ...
					i*GGSparams.pixSize(1) j*GGSparams.pixSize(2) ]';
		end;
	end;
	infoO = struct('frameNum',1,'vbl',0,'rotationangle',rotationangle,'gridvalues',V,...
			'colorvalues',colorvalues,'X',X,'Y',Y, 'gridrects',gridrects);

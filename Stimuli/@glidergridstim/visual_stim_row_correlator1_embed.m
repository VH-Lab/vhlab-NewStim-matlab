function next_frame = visual_stim_row_correlator1_embed(GGS, frame_data, correlator,sign)
% VISUAL_STIM_ROW_CORRELATOR1_EMBED - Compute the next visual stimulus frame for a given correlator
%
%  NEXT_FRAME = VISUAL_STIM_ROW_CORRELATOR1_EMBED(GGS, FRAME_DATA, CORRELATOR, SIGN)
%
%  This is the embedded version for the GLIDERGRIDSTIM function (see also
...
%      for i=1:10,
%         row_data(i+1,:) = visual_stim_row_correlator1_embed(row_data(i,:),correlator,correlator_sign);
%      end;
%      row_data,
%    
  ...
  % WARNING WARNING WARNING 
  %
  %  Edit this file with extreme care. The GLIDERGRIDSTIM uses this
  %  function with the random number generator state initialized to a 
  %  particular value. Changing the number or order of the random number
  %  calls will impact our ability to reconstruct previously saved 
  %  GLIDERGRIDSTIMs. The only parameter saved is the random number 
  %  generator state. So best not to edit this file.
  %

...
switch (PSparams.windowShape),
	...
	case 8, % gaussian
		...
		gauss_img = 1-exp(-( (X.^2)/(2*sigma_x.^2) + (Y.^2)/(2*sigma_y.^2) ) );
		img_rgba(:,:,4) = uint8(255*gauss_img);
end;
...
switch(PSparams.windowShape),
	case 6, % remove rectangular aperture
		img_rgba(:,:,4) = img_rgba(:,:,4) + uint8( (abs(X)<=0.5*PSparams.aperture(2) & abs(Y)<=0.5*PSparams.aperture(1)));
	case 7, % remove oval aperture
		img_rgba(:,:,4) = img_rgba(:,:,4) + uint8( (((X.^2)/((0.5*PSparams.aperture(2))^2) + (Y.^2)/((0.5*PSparams.aperture(1))^2))<=1    ) );
end;

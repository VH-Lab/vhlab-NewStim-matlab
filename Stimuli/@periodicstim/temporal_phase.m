...
while loops>=0, % calculate the temporal phase of each frame to be shown
		...
		if isfield(PSparams,'phaseSequence')&isfield(PSparams,'phaseSteps'),
			if ~isempty(PSparams.phaseSequence)&~isempty(PSparams.phaseSteps),
				phases_to_make_discrete = 2*pi*[0:1/PSparams.phaseSteps:1-1/PSparams.phaseSteps];
				temporal_phase = stepfunc( phases_to_make_discrete, 2*pi*(PSparams.phaseSequence-1)/PSparams.phaseSteps, temporal_phase);
			end;
		end;
